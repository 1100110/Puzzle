...
private static auto getTokenValue(ref const Token t) pure nothrow {
	return t.type < tokenValues.length ? tokenValues[t.type] : t.ptr[0 .. t.len];
}
...
public:
	...
	char* ptr;		 // _pointer to first character of this token within buffer
	size_t len;
...
struct Lexer {
	...
	*/
	Tok peekNext() {
		return this.peek(&this.token).type;
	}
	...
	void scan(Token* t) {
		...
		while (true) {
			...
			switch (*_p) {
				...
				case '{': _p++; t.type = Tok.LBrace; 	return;
				case '}': _p++; t.type = Tok.RBrace; 	return;
